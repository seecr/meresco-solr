...
class SolrServerTest(IntegrationTestCase): 
    ...
    def testJoinWithUnknownCore(self):
        ...
        header, body = getRequest(port=self.solrPort, path='/solr/records/join', arguments={'q': '*:*', 'facet': 'on', 'joinFacet.field': '{!myjoin core=core_unknown}field0'}, parse=False)
        self.assertTrue('400 Bad Request' in header, header + body)
        self.assertTrue('Cross-core join: no such core core_unknown' in body, body)
    ...
    def testFacetJoinWithoutJoinQuery(self):
        ...
        sleepWheel(2)
        header, body = getRequest(port=self.solrPort, path='/solr/records/join', arguments={'q': '*:*', 'facet': 'on', 'joinFacet.field': '{!myjoin core=core2}__id__'}, parse=True)
    ...
    def testFacetJoinOnNonQueriedCore(self):
        ...
        sleepWheel(2)
        header, body = getRequest(port=self.solrPort, path='/solr/records/join', arguments={'q': '*:*', 'join': '{!myjoin core=core3}*:*', 'facet': 'on', 'joinFacet.field': '{!myjoin core=core2}__id__'}, parse=True)
        self.assertTrue('200 OK' in header, header + tostring(body))
    ...
    def testQueryWithoutJoin(self):
        header, body = getRequest(port=self.solrPort, path='/solr/records/join', arguments={'q': '*:*'}, parse=True)
        self.assertTrue('200 OK' in header, header + tostring(body))
        self.assertEquals(10, len(xpath(body, '//result[@name="response"]/doc/str[@name="__id__"]/text()')))

    def testQueryWithJoinQueryOnSameCore(self):
        header, body = getRequest(port=self.solrPort, path='/solr/records/join', arguments={'q': '*:*', 'join': '{!myjoin core=records}__id__:record\:0001'}, parse=True)
        self.assertTrue('200 OK' in header, header)
        self.assertEquals(['record:0001'], xpath(body, '//result[@name="response"]/doc/str[@name="__id__"]/text()'))

    def testInvalidJoinQuery(self):
        header, body = getRequest(port=self.solrPort, path='/solr/records/join', arguments={'q': '*:*', 'join': '{!myjoin core=core2}field:value:other'}, parse=False)
        self.assertTrue('400 Bad Request' in header, header + body)
        self.assertTrue("Cannot parse 'field:value:other'" in body, body)
        header, body = getRequest(port=self.solrPort, path='/solr/records/join', arguments={'q': '*:*', 'join': 'field:value'}, parse=False)
        self.assertTrue('400 Bad Request' in header, header + body)
