...
class SolrShardingTest(IntegrationTestCase):
    ...
    def testSortingOnRelevancyWithShards(self):
        header, body = getRequest(port=self.solr1.solrPort,
                ...
                arguments= { "q": '__all__:fiets', 'fl': '__id__,score', "debugQuery": "on"})
        ...
        self.assertEquals(['0.22091983', '0.11045992'], body.xpath('//doc/float[@name="score"]/text()'))
    ...
    def testSortingOnRelevancyWithORquery(self):
        ...
        self.assertEquals(['record:0004', 'record:0001'], body.xpath('//doc/str[@name="__id__"]/text()'))
        self.assertEquals(['1.0479599', '0.11045992'], body.xpath('//doc/float[@name="score"]/text()'))

        header, body = getRequest(port=self.solr2.solrPort,
                path="/solr/records/select", 
                arguments= { "q": 'zadel OR stuur OR wiel OR ketting', 'fl': '__id__,score'})
        self.assertEquals(['record:0003', 'record:0002'], body.xpath('//doc/str[@name="__id__"]/text()'))
        self.assertEquals(['0.53341985', '0.22091983'], body.xpath('//doc/float[@name="score"]/text()'))
        ...
        self.assertEquals(['1.0479599', '0.53341985', '0.22091983', '0.11045992'], body.xpath('//doc/float[@name="score"]/text()'))

    def testTryToGetScoresOnCoordinationFactorOnlyAndNotTFIDF(self):
        header, body = getRequest(port=self.solr1.solrPort,
                path="/solr/records/select", 
                arguments= { "q": '__all_similarity__:zadel OR __all_similarity__:stuur OR __all_similarity__:wiel OR __all_similarity__:ketting', 'fl': '__id__,score'})
        self.assertEquals(['record:0004', 'record:0001'], body.xpath('//doc/str[@name="__id__"]/text()'))
        self.assertEquals(['4.0', '1.0'], body.xpath('//doc/float[@name="score"]/text()'))

        header, body = getRequest(port=self.solr2.solrPort,
                path="/solr/records/select", 
                arguments= { "q": '__all_similarity__:zadel OR __all_similarity__:stuur OR __all_similarity__:wiel OR __all_similarity__:ketting', 'fl': '__id__,score'})
        self.assertEquals(['record:0003', 'record:0002'], body.xpath('//doc/str[@name="__id__"]/text()'))
        self.assertEquals(['3.0', '2.0'], body.xpath('//doc/float[@name="score"]/text()'))

        header, body = getRequest(port=self.solr1.solrPort,
                path="/solr/records/select", 
                arguments= { "q": '__all_similarity__:zadel OR __all_similarity__:stuur OR __all_similarity__:wiel OR __all_similarity__:ketting', 'fl': '__id__,score',
                    "shards": 'localhost:%s/solr/records,localhost:%s/solr/records' % (self.solr1.solrPort, self.solr2.solrPort)})
        self.assertEquals(['4.0', '3.0', '2.0', '1.0'], body.xpath('//doc/float[@name="score"]/text()'))
